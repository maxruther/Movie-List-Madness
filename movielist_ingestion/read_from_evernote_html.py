    ...
    for table in soup.find_all("en-table"):
        ...
        for row in table.find_all("tr"):
            ...
            if row_num == 0:
                ...
                curr_row = [movie_id] + curr_row
                curr_entry = dict(zip(header, curr_row))

                # print(curr_row)
                if movie_dict.get(title_and_dir, -1) == -1:
                    movie_dict[title_and_dir] = curr_entry
                    movie_id += 1
                else:
                    prev_entry = movie_dict.get(title_and_dir)

                    reconciled_entry = reconcile_dup_entries(curr_entry, prev_entry)

                    curr_row = list(reconciled_entry.values())
                    # prev_row = list(prev_entry.values())
                    # # print(' ', curr_row, prev_row, '\n', sep='\n')
