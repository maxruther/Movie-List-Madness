...
BEGIN
    ...
    read_loop: LOOP
        ...
        IF @table LIKE '%backburning%' THEN
			SET @b = CONCAT('INSERT INTO allWatched SELECT title, release_date, director, watched, 0, rating, date_watched, native_ordering FROM ', @table, ' WHERE Watched = 1 AND CONCAT(Title,Director) NOT IN (SELECT CONCAT(Title,Director) FROM allWatched)');
		ELSE
			SET @b = CONCAT('INSERT INTO allWatched SELECT title, release_date, director, watched, Watched_in_theater, rating, date_watched, native_ordering FROM ', @table, ' WHERE Watched = 1 AND CONCAT(Title,Director) NOT IN (SELECT CONCAT(Title,Director) FROM allWatched)');
		END IF;
